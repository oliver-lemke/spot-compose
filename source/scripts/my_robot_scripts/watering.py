...
from robot_utils.advanced_movement import move_body_distanced, positional_grab
    ...
    def __call__(
        ...
        **kwargs,
        ...
        robot_target = body_planning(
            environment_cloud, item_coordinates, min_distance=0.85, max_distance=0.85
        )[0]
        ...
        # rotation_pose.set_rot_from_angle(150, degrees=True)
        # move_body(rotation_pose, frame_name)
        watering_coordinates = Pose3D(STAND_COORDINATES)
        move_body_distanced(watering_coordinates.to_dimension(2), 1, frame_name)
        watering_coordinates.rot_matrix = tf_matrix[:3, :3]

        correct_roll_matrix = Rotation.from_euler(
            "xyz", (90, 0, 0), degrees=True
        ).as_matrix()
        roll = Pose3D(rot_matrix=correct_roll_matrix)
        watering_coordinates = watering_coordinates @ roll
        move_arm(watering_coordinates, frame_name)
