...
from robot_utils.advanced_movement import dynamically_refined_grasp_renew_grasp
...
class _RefinedGrasp(ControlFunction):
    def __call__(
        self,
        config: Config,
        sdk: Sdk,
        ...
        **kwargs,
        ...
        ITEM, INDEX, BOOL = "coffee table", 1, True
        ITEM, INDEX, BOOL = "object", 2, True
        ...
        robot_target = body_planning(
            environment_cloud, item_coordinates, min_distance=0.75, max_distance=0.75
        )[0]
...
def main():
    config = Config()
    take_control_with_function(config, function=_RefinedGrasp(), body_assist=True)
