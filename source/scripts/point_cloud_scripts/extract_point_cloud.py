...
def get_point_cloud_data_in_seed_frame(waypoints, snapshots, anchorings, waypoint_id):
    ...
    if waypoint_id not in anchorings:
        raise Exception(
            f"{waypoint_id} not found in anchorings. Does the map have anchoring data?"
        )
    seed_tform_cloud = (
        SE3Pose.from_proto(anchorings[waypoint_id].seed_tform_waypoint)
        * waypoint_tform_cloud
    )
    point_cloud_data = np.frombuffer(cloud.data, dtype=np.float32).reshape(
        ...
        for waypoint in current_graph.waypoints:
            ...
            with open(file_name, "rb") as snapshot_file:
                ...
                for fiducial in waypoint_snapshot.objects:
                    ...
                    if (
                        str_id in current_anchored_world_objects
...
def main(argv):
    ...
    parser.add_argument("--path", type=str, help="Map to extract.", default=data_path)
    parser.add_argument(
        "--output", type=str, help="Output PLY file.", default=point_cloud_path
    )

    pc_dirname = os.path.dirname(point_cloud_path)
    os.makedirs(pc_dirname, exist_ok=True)
