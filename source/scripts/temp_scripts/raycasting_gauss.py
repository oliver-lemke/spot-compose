...
def oddize_kernel_size(kernel: (float, float)) -> (float, float):
    def add_one_if_odd(nr: int) -> int:
        if nr % 2 == 0:
            nr += 1
        return nr

    new_kernel = [add_one_if_odd(knr) for knr in kernel]
...
def main() -> None:
    ...
    ADD_BELOW = False
    RES = 128
    KERNEL_SIZE = (RES // 3, RES // 3)
    KERNEL_SIZE = oddize_kernel_size(KERNEL_SIZE)
    sigmas = {"sigmaX": RES / 4, "sigmaY": RES / 2}
    PLOT_PROGRESSION = False
    RAY_START_COORDINATES = (-0.5, 0, 0.1)
    ...
    angles, distances_rep = repeat_scene(angles, distances, below=ADD_BELOW)
    _, valid_points = repeat_scene(angles, valid_points, below=ADD_BELOW)

    distances_image = distances_rep.astype(np.uint8)
    distances_image[valid_points] += 2
    blurred_distances = cv2.GaussianBlur(distances_image, KERNEL_SIZE, **sigmas)
    blurred_distances[~valid_points] = 0
    if PLOT_PROGRESSION:
        plot_ray_scene(angles, blurred_distances)

    # extract single image
    height, width = distances.shape[:2]
    original_angles = angles[height : 2 * height, width : 2 * width]
    original_values = blurred_distances[height : 2 * height, width : 2 * width]
